...
<section class="container mx-auto p-4">
    ...
    <form action="{{ route('search') }}" method="GET" class="pagination">
        <button type="submit" name="page" value="{{ $search_pagination_current - 1 }}"
            ...
            < </button>
                ...
                @for ($i = $search_pagination_current - 1; $i < $search_pagination_current + 9; $i++) <button
                    type="submit" name="page" value="{{ $i + 1 }}"
                    class="md:block p-2 w-32 h-full text-lg hover:bg-rose-300 {{ $search_pagination_current == $i + 1 ? 'bg-rose-300' : 'bg-rose-200 hidden' }}">
                    {{ $i + 1 }}
        </button>
        @endfor
        ...
        @for ($i = 0; $i < $search_pagination_total; $i++) <button type="submit" name="page" value="{{ $i + 1 }}"
            ...
            <button name="page" value="{{ $search_pagination_current }}"
                class="md:hidden block p-2 w-32 h-full text-lg hover:bg-rose-300 bg-rose-300">{{
                $search_pagination_current }}</button>
            @endif
            <button type="submit" name="page" value="{{ $search_pagination_current + 1 }}"
    ...
    <div class="md:flex">
        <div class="search-results__container search-results__container--mobile tab-container tab-container--search-results md:hidden">
            <div class="tab-menu">
                <ul>
            ...
            <div class="tab tab-active" data-id="tab1">
                <div class="search-results mb-4">
                    ...
                    <div class="search-results__shows">
                        ...
                        <div
                            class="search-result__cover border border-2 border-stone-600 p-2 mb-1 hover:bg-stone-200 flex flex-col justify-center items-center">
                            @if (isset($item->poster_path))
                            <img class="search-result__image max-w-5/10 mb-2"
                                src="https://image.tmdb.org/t/p/w500/{{ $item->poster_path }}" alt="">
                            @else
                            <img class="search-result__image max-w-5/10 mb-2" src="../images/placeholders/no_postah.jpg"
                                alt="">
                            @endif
                            @if (isset($item->in_calendar) && $item->in_calendar)
                            <a href="{{ route('users.tv.delete', $item->id) }}"
                                class="inline-block mb-2 hover:text-rose-500 text-rose-400">
                                <img class="w-16 h-16" src="../images/icons/solid/heart.svg" alt="">
                            </a>
                            @else
                            <a href="{{ route('users.tv.store', $item->id) }}"
                                class="inline-block mb-2 hover:text-rose-400">
                                <img class="w-16 h-16" src="../images/icons/outline/heart.svg" alt="">
                            </a>
                            @endif
                ...
                <div class="search-results mb-4">
                    ...
                    <div class="search-results__movies">
                        ...
                        <div
                            class="search-result__cover border border-2 border-stone-600 p-2 mb-1 hover:bg-stone-200 flex flex-col justify-center items-center">
                            @if (isset($item->poster_path))
                            <img class="search-result__image max-w-5/10 mb-2" src="https://image.tmdb.org/t/p/w500/{{ $item->poster_path }}" alt="">
                            @else
                            <img class="search-result__image max-w-5/10 mb-2" src="../images/placeholders/no_postah.jpg"
                                alt="">
                            @endif
    ...
    <div class="search-results__container search-results__container--desktop">
        <div class="hidden md:flex justify-center search-results search-results mb-4">
            @if ($search_results->tv)
            <div class="mr-2 max-w-5/10 w-[50%] search-results__shows">
                ...
                <div class="search-result__cover border border-2 border-stone-600 p-2 mb-1 hover:bg-stone-200">
                    ...
                    <div class="flex items-center">
                        @if (isset($item->in_calendar) && $item->in_calendar)
                        <a href="{{ route('users.tv.delete', $item->id) }}" class="inline-block hover:text-rose-400">
                            <img class="w-8 h-8 mr-4" src="../images/icons/solid/heart.svg" alt="">
                        </a>
                        @else
                        <a href="{{ route('users.tv.store', $item->id) }}" class="inline-block hover:text-rose-500 text-rose-400">
                            <img class="w-8 h-8 mr-4" src="../images/icons/outline/heart.svg" alt="">
                        </a>
                        @endif
                        <a href="{{ route('tv.show', $item->id) }}">
                            <div>
            ...
            <div class="ml-2 max-w-5/10 w-[50%] search-results__movies">
                ...
                <div class="search-result__cover border border-2 border-stone-600 p-2 mb-1 hover:bg-stone-200">
                    ...
                    <div class="flex items-center">
                        @if (isset($item->in_calendar) && $item->in_calendar)
                            <a href="{{ route('users.movie.delete', $item->id) }}" class="inline-block hover:text-rose-400">
                                <img class="w-8 h-8 mr-4" src="../images/icons/solid/heart.svg" alt="">
                            </a>
                        @else
                        <a href="{{ route('users.movie.store', $item->id) }}" class="inline-block hover:text-rose-500 text-rose-400">
                            <img class="w-8 h-8 mr-4" src="../images/icons/outline/heart.svg" alt="">
                        </a>
                        @endif
                        <a href="{{ route('movies.show', $item->id) }}">
            ...
            <div
                ...
                <div class="mt-2">
                    ...
                    <div class="font-bold mb-1 hover:bg-stone-200 py-2 px-1">
                        @if ($item['type'] == 'movie')
                        <a href="{{ route('movies.show', $item['id']) }}}" class="leading-5">{{ $item['name'] }}</a>
                        @else
                        <a href="{{ route('tv.show', $item['id']) }}">{{ $item['show_name'] }}</a>
                        @endif
                    </div>
                    @endforeach
                </div>
